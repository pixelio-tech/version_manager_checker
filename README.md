...
A/B тесты заводятся в админке (раздел «A/B тесты» приложения) и с флагами
не связаны: флаг — выключатель фичи, тест — сравнение вариантов. У теста
свои параметры (например, `layout`, `trial_days`, `config`), каждый вариант
задаёт им свои значения. Сервер сам делит устройства по вариантам и держит
назначение: устройство остаётся в своём варианте и после обновления.
...
final config = paywall.getJson('config', const {});
```

Второй аргумент — значение из кода. Его приложение получает, когда
устройство не в тесте (не подошло под аудиторию, тест не запущен или на
паузе, сервер ещё не отвечал), когда параметра нет или его тип не совпал.
Значения по умолчанию в коде должны совпадать с контролем.

Нужен только вариант, без параметров, — `paywall.variant` (`null` — не в
тесте). `paywall.isActive` и карта `vm.experiments` экспозицию не отмечают.

Первое за запуск чтение варианта или параметра отправляет на сервер
экспозицию: отчёт видит, сколько назначенных дошли до экрана с тестом.
Поэтому читайте параметры там, где они влияют на экран, а не при старте.
Если значения нужны заранее, а экран покажется позже, — `paywall.logExposure()`
в момент показа.

Когда в админке выкатывают победителя, тест продолжает приходить с
параметрами победителя всем из его аудитории (`isShipped == true`),
экспозиция больше не отправляется. Код менять не нужно; перенести значения
в код можно в своём темпе, после этого тест архивируют.

Сообщения тоже можно сравнивать: у варианта в админке может быть своё
сообщение. Приложению делать ничего не нужно — сервер присылает сообщение
нужного варианта, `presentAll` его показывает, показ отмечает экспозицию.

#### Цель — событие приложения

Обновление версии и возврат сервер видит сам. Всё остальное — покупку,
пройденный онбординг — приложение сообщает событием, а в админке это
событие ставится целью теста:

```dart
vm.track('purchase', value: 9.99); // value — для метрики «сумма на участника»
vm.track('onboarding.done');
```

Имя — латиница в нижнем регистре, цифры, `_` и `.`. `track` не ждёт сети:
событие ложится в очередь, очередь хранится в `VmStorage`, переживает
перезапуск и офлайн (до 500 событий, дальше выбрасываются самые старые) и
уходит пачками — при заполнении, раз в 30 секунд и после каждой проверки
версии. Перед уходом приложения в фон можно отправить сразу:
`await vm.flushEvents()`.

#### Посмотреть вариант на своём телефоне

В админке у теста есть «Тестовые устройства»: `instanceId` устройства и
вариант, который ему показывать. Работает и у черновика — вариант видно до
запуска, в отчёт такие устройства не попадают. `instanceId` своего
устройства — `vm.instanceId`; удобно вывести его на отладочный экран.
