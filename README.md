...
### Необязательное обновление: как часто напоминать
...
Режим задаёт админка (`recommendedVersion.frequency`): всегда, один раз на
установку, каждый N-й запуск или не чаще раза в N часов. Считает его клиент —
сервер видит проверки, а за один старт их бывает несколько (поллинг, возврат
из фона), и «каждый третий запуск» превратился бы в «каждую третью проверку».
Счётчик запусков растёт один раз за `init()` и лежит в `VmStorage`, поэтому
переживает перезапуск; `markUpdateReminderShown()` сдвигает паузу, без него
режимы стоят на месте.

Новая рекомендованная сборка сбрасывает счётчики: пропущенные запуски
относились к прошлому обновлению.

Обязательного обновления это не касается — `VmUpdateGate` показывает его
всегда, частота про вежливое напоминание.
